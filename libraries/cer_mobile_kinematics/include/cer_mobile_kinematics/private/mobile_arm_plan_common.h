...
/****************************************************************/
class MobileArmPlanCommonNLP : public TripodNLPHelper,
                     public Ipopt::TNLP
{
protected:
    MobileArmPlanSolver &slv;
    ...
    double domain_dist;
    double out_domain_height;
    double navigation_margin;
    double obstacle_margin;
    vector<double> obs_dist;
    ...
    vector<vector<int>> checkPoint_idx;
    ...
    TripodState tripod_fkin(const int which, const Ipopt::Number *x,
                            TripodState *internal=NULL, const int point_idx=0)
    {
        const TripodParametersExtended &params=((which==1)?torso:lower_arm);
        int offs=(which==1)?idx_t[point_idx]:idx_la[point_idx];
...
public:
    /****************************************************************/
    MobileArmPlanCommonNLP(MobileArmPlanSolver &slv_, int nb_targets_=1, int nb_points_=0) :
                 ...
                 obstacle_margin(0)
    {
        ...
        yAssert(nb_checkPoints>=0);

        nb_points_per_target=1+nb_checkPoints;
        nb_total_points=nb_targets*nb_points_per_target;

        obs_dist.resize(nb_total_points,0);

        idx_b=0;
        idx_t.resize(nb_total_points);
        idx_t[0]=idx_b+3;
        idx_ua.resize(nb_total_points);
        idx_ua[0]=idx_t[0]+3;
        idx_la.resize(nb_total_points);
        ...
        checkPoint_idx.resize(nb_targets);
        target_idx.resize(nb_targets);
        for (size_t i=0; i<nb_targets; i++)
        {
            checkPoint_idx[i].resize(nb_checkPoints);
            for (size_t j=0; j<nb_checkPoints; j++)
            {
                checkPoint_idx[i][j]=i*nb_points_per_target+j;
                yDebug() << "c" << checkPoint_idx[i][j];
            }

            target_idx[i]=(i+1)*nb_points_per_target-1;
            yDebug() << "t" << target_idx[i];
        }
        for (size_t i=0; i<nb_total_points; i++)
        {
            yDebug() << idx_t[i] << idx_ua[i] <<idx_la[i];
        }
        ...
        xref=x0;
        x.resize(3+nb_total_points*nb_kin_DOF);
        ...
        d1.resize(nb_total_points);
        d2.resize(nb_total_points);
        din1.resize(nb_total_points);
        din2.resize(nb_total_points);
        H.resize(nb_total_points,eye(4,4));
        H_.resize(nb_total_points,eye(4,4));
        J_.resize(nb_total_points,eye(6,upper_arm.getDOF()));
        T.resize(nb_total_points,eye(4,4));
        xe.resize(nb_total_points,Vector(3,0.0));
        T0=eye(4,4);

        q.resize(nb_total_points, x0.subVector(idx_ua[0],idx_ua[0]+upper_arm.getDOF()-1));
    ...
    /****************************************************************/
    virtual void set_q0(const Vector &q0)
    {
        for (size_t i=0; i<3; i++)
            x0[idx_b+i]=q0[idx_b+i];

        for (size_t i=0; i<3; i++)
            x0[idx_t[0]+i]=std::max(torso.l_min,std::min(torso.l_max,q0[idx_t[0]+i]));

        iKinChain *chain=upper_arm.asChain();
        for (size_t i=0; i<upper_arm.getDOF(); i++)
            x0[idx_ua[0]+i]=std::max((*chain)[i].getMin(),std::min((*chain)[i].getMax(),CTRL_DEG2RAD*q0[idx_ua[0]+i]));

        for (size_t i=0; i<3; i++)
            x0[idx_la[0]+i]=std::max(lower_arm.l_min,std::min(lower_arm.l_max,q0[idx_la[0]+i]));

        Ipopt::Number x0_[x0.size()];
        for (size_t i=0; i<x0.size(); i++)
            x0_[i]=x0[i];
        TripodState d1_=tripod_fkin(1,x0_,nullptr,0);
        TripodState d2_=tripod_fkin(2,x0_,nullptr,0);

        upper_arm.setH0(H0); upper_arm.setHN(HN);
        T0=d1_.T*upper_arm.getH(x0.subVector(idx_ua[0],idx_la[0]-1))*d2_.T*TN;
        yDebug() << T0.toString();
    }
    ...
    /****************************************************************/
    virtual void set_out_domain_height(double height)
    {
        out_domain_height=height;
    }

    /****************************************************************/
    virtual void set_navigation_margin(double margin)
    {
        navigation_margin=margin;
    }

    /****************************************************************/
    virtual void set_obstacle_margin(double margin)
    {
    ...
    virtual Vector get_result() const
    {
        ...
        x_[idx_b+2]=remainder(CTRL_RAD2DEG*x_[idx_b+2], 360.0);
    ...
    virtual void computeQuantities(const Ipopt::Number *x, const bool new_x)
    {
        if (new_x)
        {
            ...
            if(domain_constr)
            {
                ...
                for(size_t i=0; i<nb_total_points; i++)
                {
                    obs_dist[i]=distanceFromOutDomainObstacle(domain_poly, out_domain_height, xe[i]);
//yDebug() << obs_dist[i];
                }
            }
        }
    ...
    double distanceFromOutDomainObstacle(const std::vector<Vector> &domain, double height, const Vector &point)
    {
        double obs_dist2D=navigation_margin+distanceFromDomain(domain, point.subVector(0,1));

        double h=point[2]-height;

        // Object outside domain
        if(obs_dist2D<0)
            return std::max(obs_dist2D, h);
        // Object inside domain and lower than obstacle height
        else if(h<0)
            return obs_dist2D;
        // Object inside domain and higher than obstacle height
        else
            return sqrt(obs_dist2D*obs_dist2D+h*h);
    }
};

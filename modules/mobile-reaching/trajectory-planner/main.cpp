...
public:
    ...
    bool configure(ResourceFinder &rf)
    {
        ...
        rpcPort.open("/cer_mobile-reaching-planner/"+arm_type+"/rpc");
        attach(rpcPort);
    ...
    bool respond(const Bottle &cmd, Bottle &reply)
    {
        ...
        if (cmd.check("get"))
        {
            ...
            Bottle &nb_check_points=payLoad2.addList();
            nb_check_points.addString("nb_check_points");
            nb_check_points.addDouble(solver.getNbCheckPoints());

            Bottle &out_domain_height=payLoad2.addList();
            out_domain_height.addString("out_domain_height");
            out_domain_height.addDouble(solver.getOutDomainHeight());

            Bottle &obstacle_margin=payLoad2.addList();
            obstacle_margin.addString("obstacle_margin");
            obstacle_margin.addDouble(solver.getObstacleMargin());
        ...
        if (cmd.check("parameters"))
        {
            if (Bottle *parameters=cmd.find("parameters").asList())
            {
                ...
                if (parameters->check("nb_check_points"))
                {
                    solver.setNbCheckPoints(parameters->find("nb_check_points").asDouble());
                    if(verbosity>0)
                        yInfo() << "Number of checkpoints set:" << solver.getNbCheckPoints();
                    ack=true;
                }

                if (parameters->check("navigation_margin"))
                {
                    solver.setNavigationMargin(parameters->find("navigation_margin").asDouble());
                    if(verbosity>0)
                        yInfo() << "Navigation margin set:" << solver.getObstacleMargin();
                    ack=true;
                }

                if (parameters->check("out_domain_height"))
                {
                    solver.setOutDomainHeight(parameters->find("out_domain_height").asDouble());
                    if(verbosity>0)
                        yInfo() << "Height of out domain obstacle set:" << solver.getOutDomainHeight();
                    ack=true;
                }

                if (parameters->check("obstacle_margin"))
                {
                    solver.setObstacleMargin(parameters->find("obstacle_margin").asDouble());
                    if(verbosity>0)
                        yInfo() << "Obstacle margin set:" << solver.getObstacleMargin();
                    ack=true;
                }
        ...
        if (Bottle *targetList=cmd.find("target").asList())
        {
            ...
            int nb_kin_DOF = (q.size()-3)/((1+solver.getNbCheckPoints())*Hd.size());
            for(size_t i=0; i<(1+solver.getNbCheckPoints())*Hd.size(); i++)
            {
